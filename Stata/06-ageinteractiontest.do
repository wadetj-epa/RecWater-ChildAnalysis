...
set more off
capture log close
log using "C:\Users\twade\OneDrive - Environmental Protection Agency (EPA)\Rec_Water\ChildAnalysis\Results\interactiontests\giinteractiontests.log", replace
...
putexcel set "C:\Users\twade\OneDrive - Environmental Protection Agency (EPA)\Rec_Water\ChildAnalysis\Results\interactiontests\giinteractiontests.xlsx", modify
...
*foreach site in risknotrop neearall notropical neearps neearcore risk{
*foreach site in risk {
	foreach swim in anycontact bodycontact swallwater water30 water45 water60{
		foreach exp in avgdyenteropcr avgdyentero1600 {
			foreach gi in hcgi diarrhea nausea stomach severegi {
				foreach age in age12 age10 age8 age6 age4 {
					foreach subset in all adults {
				
						local j=`j'+1
						*local j=`count'+1
						selectaic logistic if `site'==1 & `swim'==1, outcome(`gi') exposure(`exp') covar(`covars') keepvar(i.beachg) stat(AIC) options(vce(cluster hhid))
						local vars `r(covarlist)'
						
						if "`subset'"=="all" {
							logistic `gi' i.`age'##c.`exp' `vars' i.beachg if `site'==1 & `swim'==1, vce(cluster hhid)
						}
						if "`subset'"=="adults" {
							logistic `gi' i.`age'##c.`exp' `vars' i.beachg if `site'==1 & `swim'==1 & (`age'==1 | age>=18), vce(cluster hhid)
						}
